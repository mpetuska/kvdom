...
abstract class VNode<T : Node>(internal val uuid: Long = UUID.next(), node: T? = null) {
    protected var `$node`: T? = node; private set
    internal var snapshot: VNode<T>? = null; private set

    internal abstract fun diff(): () -> T?
    ...
    internal abstract fun render(): T
     ...
     */
    protected open fun onPrePatch(oldVnode: VNode<T>?) {}
     ...
     */
    protected open fun onUpdate(oldVnode: VNode<T>?) {}

    /**
     * Called after both, vNode and DOM node has been updated.
     */
    protected open fun onPostPatch(oldVnode: VNode<T>?) {}
